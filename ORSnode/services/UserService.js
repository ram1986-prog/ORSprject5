...
class UserService extends BaseService {
    ...
    add(user, callback, ctx) {
        ...
        var sql = "INSERT INTO st_user (CREATED_DATETIME,MODIFIED_DATETIME,FIRST_NAME,LAST_NAME,LOGIN,PASSWORD,MOBILE_NO,ROLE_ID,ROLE_NAME,GENDER) "
            + " VALUES (NOW(),NOW(),?,?,?,?,?,?,?,?)";
        var params = [user.firstName, user.lastName, user.login,
        user.password, user.mobileNo, user.roleId,user.roleName, user.gender];
    ...
    update(user, callback, ctx) {
        var sql = "UPDATE st_user SET MODIFIED_DATETIME = NOW(), FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,ROLE_NAME=?,GENDER=? WHERE ID=?"
        var params = [user.firstName, user.lastName, user.login,
        user.password, user.dob, user.mobileNo, user.roleId,user.roleName, user.gender, user.id];
        super.executeSQL(sql, params, function (err, result) {
            if (err) {
                callback(err);
            } else {
