...
class StudentService extends BaseService {
    ...
    add(student, callback, ctx) {
        var sql = "INSERT INTO st_student (CREATED_DATETIME,MODIFIED_DATETIME,COLLEGE_ID,COLLEGE_NAME,FIRST_NAME,LAST_NAME,DOB,MOBILE_NO,EMAIL_ID) "
            + " VALUES (NOW(),NOW(),?,?,?,?,?,?,?)";
        var params = [student.collegeId, student.collegeName, student.firstName,
        student.lastName, student.dob, student.mobileNo, student.emailId];
    ...
    update(student, callback, ctx) {
        var sql = "UPDATE st_student SET  MODIFIED_DATETIME = NOW(), COLLEGE_ID=?,COLLEGE_NAME=?,FIRST_NAME=?,LAST_NAME=?,DOB=?,MOBILE_NO=?,EMAIL_ID=?  WHERE ID=?"
        var params = [student.collegeId, student.collegeName, student.firstName,
        student.lastName, student.dob, student.mobileNo, student.emailId, student.id];
        super.executeSQL(sql, params, function (err, result) {
            if (err) {
                callback(err);
            } else {
